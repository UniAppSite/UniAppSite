...
import { getFirestore, collection, addDoc, getDocs, doc, deleteDoc, onSnapshot, query, orderBy, where } from "https://www.gstatic.com/firebasejs/11.5.0/firebase-firestore.js";
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        window.location.href = "https://uniapp.site/Chat/Login.html"; // Redirect to login
    }
});
...
logoutBtn.addEventListener("click", () => {
    signOut(auth).then(() => {
        window.location.href = "https://uniapp.site/Chat/Login.html";
    }).catch((error) => {
